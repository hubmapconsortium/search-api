...
class App:
    ...
    def main(self):
        try:
            ...
            donors = requests.get(conf.ENTITY_WEBSERVICE_URL + "/entities?entitytypes=Donor").json()
    ...
    def index_tree(self, donor):
        descendants = requests.get(conf.ENTITY_WEBSERVICE_URL + "/entities/descendants/" + donor.get('uuid', None)).json()
