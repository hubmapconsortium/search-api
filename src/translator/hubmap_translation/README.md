...
````
python3 -m translator.hubmap_translator.py <globus-nexus-token>
````
...
````
python3 -m translator.hubmap_translator.py <globus-nexus-token> 1>indexer.log 2>&1
````
...
```
PYTHONPATH=. translator/hubmap_translation/debug.py ~/failing-doc-1.yaml ~/failing-doc-2.json ...
```
